...
const SCHEMA = {
  ...
  tipos: {
    AGENDA:     { tipo:"select:Culto,Reunião,Ensaio,Aula,Congresso,Conferência,Evento,Casamento,Aniversário,Outros", data:"date", hora_inicio:"time", hora_fim:"time", dia_semana:"select:Domingo,Segunda-feira,Terça-feira,Quarta-feira,Quinta-feira,Sexta-feira,Sábado", mes:"select:Janeiro,Fevereiro,Março,Abril,Maio,Junho,Julho,Agosto,Setembro,Outubro,Novembro,Dezembro", status:"select:pendente=Pendente,aguardando_aprovacao=Aguardando Aprovação,em_analise=Em Análise,ajuste_solicitado=Ajuste Solicitado,confirmado=Confirmado,realizado=Realizado,reagendado=Reagendado,cancelado=Cancelado,recusado=Recusado,arquivado=Arquivado", recorrencia:"select:Único,Semanal,Quinzenal,Mensal,Anual,Data Especial", espaco:"espacos-multi" },
    MEMBROS:    { status:"select:ativo,inativo,transferido,falecido,disciplinado,afastado", tipo_membro:"select:=Nenhum,comungante=Comungante,nao_comungante=Não Comungante", data_nascimento:"date", data_ingresso:"date", data_saida:"date", data_batismo:"date", batizado:"boolean", casado_na_igreja:"boolean", tipo_ingresso:"select:batismo=Batismo,transferencia=Transferência,profissao_de_fe=Profissão de Fé,restauracao=Restauração,outro=Outro", congregacao:"congregacoes-select", funcao:"select:membro=Membro,pastor=Pastor,presbitero=Presbítero,diacono=Diácono,supervisor=Supervisor,coordenador=Coordenador,lider_ministerio=Líder de Ministério,lider_pg=Líder de PG,secretario=Secretário(a),tesoureiro=Tesoureiro(a),colaborador=Colaborador,colaborador_membro=Colaborador - Membro" },
    VISITANTES: { data_primeira_visita:"date", interesse_nivel:"select:baixo,médio,alto,convertido" },
    DEMANDAS:   { data_abertura: "date", data_conclusao: "date", status: "select:ABERTA=Pendente,EM_ANALISE=Em Análise,EM_ANDAMENTO=Em Andamento,PENDENTE=Pendente,CONCLUIDA=Concluída,CANCELADA=Cancelada", prioridade: "select:Baixa,Média,Alta,Urgente", subcategoria: "subcats-from-area" },
  ...
  fullWidth: {
    AGENDA: ["titulo","espaco","observacao"],
    MEMBROS: ["nome"]
  },
...
function renderModuloList(rows, tab, containerId) {
  ...
  const COL_PT = {titulo:"Título",data:"Data",mes:"Mês",dia_semana:"Dia da Semana",hora_inicio:"Início",hora_fim:"Fim",recorrencia:"Recorrência",espaco:"Espaço",organizador:"Organizador",observacao:"Observação",status:"Status",tipo:"Tipo",nome:"Nome",email:"E-mail",telefone:"Telefone",celular:"Celular",cargo:"Cargo",ministerio:"Ministério",area:"Área",descricao:"Descrição",valor:"Valor",quantidade:"Qtd",pgs:"PGS",lider:"Líder",endereco:"Endereço",data_batismo:"Batismo",data_membro:"Membro desde",data_entrada:"Entrada",data_saida:"Saída",categoria:"Categoria",prioridade:"Prioridade",responsavel:"Responsável",solicitante:"Solicitante",item:"Item",unidade:"Unidade",estoque_minimo:"Mín.",localizacao:"Localização",congregacao:"Vínculo",tipo_membro:"Tipo de Membro",tipo_ingresso:"Tipo de Ingresso",data_ingresso:"Data de Ingresso",data_nascimento:"Nascimento",numero_registro:"Nº Registro",funcao:"Função"};
...
const VIEW_AUTOLOAD = {
  ...
  "admin-facial":   { fn: () => { typeof afCarregar === 'function' && afCarregar(); typeof afCarregarSolicitacoes === 'function' && afCarregarSolicitacoes(); } },
  "config-usuarios":  { fn: () => carregarUsuarios() },
  "config-whatsapp":  { fn: () => typeof WA_CFG    !== "undefined" && WA_CFG.refresh() },
  "wa-listas":        { fn: () => typeof WA_LISTAS !== "undefined" && WA_LISTAS.init() },
...
async function apiWrite(action, sheet, payload) {
  ...
  if (sheet === "DEMANDAS") {
    const STATUS_DEMANDA_MAP = {
      "Aberta":"PENDENTE","aberta":"PENDENTE",
