...
async function salvarRegistro(tab, recordId = null) {
  ...
  // Normalização de status para DEMANDAS — banco só aceita valores em maiúsculas
  if (tab === "DEMANDAS") {
  ...
  try {
    // MEMBROS update: split entre tabelas pessoas e membros (v_membros é read-only)
    if (tab === "MEMBROS" && recordId) {
      ...
      // numero_registro: só inclui se preenchido (coluna tem unique constraint)
...
async function deletarRegistro(tab, recordId) {
  ...
  if (["MEMBROS","VISITANTES"].includes(tab) && USUARIO_ATUAL?.perfil !== "ADMINISTRADOR_GERAL") {
    T("Acesso negado", "Apenas o Administrador Geral pode excluir membros."); return;
  }
