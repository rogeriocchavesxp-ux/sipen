...
<body>
  ...
  async function renderDiacDash() {
    const _set = (id, v) => { const e = document.getElementById(id); if (e) e.textContent = String(v ?? '—'); };
    const api = _api(), hdrs = _hdrs();
    if (!api) return;
    try {
      const hoy = new Date();
  ...
  if(typeof VIEW_AUTOLOAD!=='undefined'){
    VIEW_AUTOLOAD['diac-dash']={fn: renderDiacDash};
  }
