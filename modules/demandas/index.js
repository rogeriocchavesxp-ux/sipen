          ...
          <tbody>
            ${rows.map(r => {
              ...
              <tr id="fin-dem-row-${rid}" style="border-bottom:1px solid var(--bd1);background:${sel?"rgba(58,170,92,.06)":""}"
                ...
                <td style="padding:8px 6px;color:var(--tx1);max-width:200px;white-space:nowrap;cursor:pointer" onclick="demAbrirDetalhe('${r.id||r._row}','fin-demandas')"><span style="display:flex;align-items:center;gap:4px"><span style="overflow:hidden;text-overflow:ellipsis">${escapeHtml(r.titulo)||"—"}</span>${(()=>{const fd=r.financial_data;return(fd&&(fd.notas_fiscais?.length||fd.nota_fiscal?.storage_path||fd.comprovante?.storage_path||fd.boleto?.storage_path))?'<span title="Tem anexo" style="font-size:11px;color:var(--teal);flex-shrink:0">📎</span>':''})()}</span></td>
