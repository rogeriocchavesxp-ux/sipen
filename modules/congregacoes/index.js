...
let _activeTab=0;          // mantido para compat com irParaSecaoCong
let _congTabAtual='visao-geral'; // ID da aba ativa

// Definição de abas com ícones SVG (mesmo padrão dos Ministérios)
const _S=(p)=>`<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" style="margin-right:5px;vertical-align:-1px">${p}</svg>`;
const _CONG_TABS=[
  {id:'visao-geral', label:'Visão Geral',     icon:_S('<path d="m3 9 9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/>'),          always:true},
  {id:'membresia',   label:'Membresia',        icon:_S('<path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/>'), always:true},
  {id:'lideranca',   label:'Liderança',        icon:_S('<circle cx="12" cy="8" r="4"/><path d="M20 21a8 8 0 0 0-16 0"/><path d="m16 11 1.5 1.5L20 10"/>'),              always:true},
  {id:'cultos',      label:'Cultos e Oração',  icon:_S('<rect width="18" height="18" x="3" y="4" rx="2"/><line x1="16" x2="16" y1="2" y2="6"/><line x1="8" x2="8" y1="2" y2="6"/><line x1="3" x2="21" y1="10" y2="10"/>'), key:'cultos'},
  {id:'pgs',         label:'Pequenos Grupos',  icon:_S('<path d="m3 9 9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/>'),        key:'pgs'},
  {id:'ministerios', label:'Ministérios',      icon:_S('<path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/>'), key:'ministerios'},
  {id:'financeiro',  label:'Financeiro',       icon:_S('<line x1="18" x2="18" y1="20" y2="10"/><line x1="12" x2="12" y1="20" y2="4"/><line x1="6" x2="6" y1="20" y2="14"/>'),key:'financeiro'},
  {id:'desafios',    label:'Desafios',         icon:_S('<path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3z"/><line x1="12" x2="12" y1="9" y2="13"/><line x1="12" x2="12.01" y1="17" y2="17"/>'), key:'desafios'},
...
const _IDX_TO_TAB=['visao-geral','membresia','cultos','pgs','ministerios','lideranca','financeiro','desafios','planejamento','departamentos','departamentos'];
const _CONG_TAB_NOMES=_CONG_TABS.map(t=>t.label);
...
function irParaSecaoCong(i){
  const tabId=_IDX_TO_TAB[i]||'visao-geral';
  _activeTab=i;
  _congTabAtual=tabId;
  ...
  document.querySelectorAll("#cong-tabs-bar .min-tab").forEach(b=>{
    b.classList.toggle("active",b.dataset.ctab===tabId);
  });
  const cong=CONG.getCong(_activeCongId);
  if(cong) _renderCongTabContent(tabId,cong);
}
...
function renderCongView(cong){
  ...
  const _tabVis=(t)=>t.always||(t.adm&&podeEd)||(!t.always&&!t.adm&&rec[t.key]!==false);
  const tabsHtml=_CONG_TABS.map(t=>{
    if(!_tabVis(t)) return `<button class="min-tab" data-ctab="${t.id}" id="cong-tab-btn-${t.id}" onclick="switchCongTabNamed('${t.id}')" style="display:none">${t.icon}${t.label}</button>`;
    return `<button class="min-tab${_congTabAtual===t.id?" active":""}" data-ctab="${t.id}" id="cong-tab-btn-${t.id}" onclick="switchCongTabNamed('${t.id}')">${t.icon}${t.label}</button>`;
  }).join("");
    ...
    <div class="card" id="cong-ver-header" style="margin-bottom:16px">
      <div style="display:flex;align-items:center;gap:16px;flex-wrap:wrap">
        <div style="display:flex;align-items:center;gap:12px;flex:1;min-width:200px">
          <div style="width:44px;height:44px;border-radius:10px;background:rgba(58,170,92,.12);border:1px solid rgba(58,170,92,.2);display:flex;align-items:center;justify-content:center;font-size:22px;flex-shrink:0">${cong.identificacao.icon||"⛪"}</div>
          <div>
            <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">
              <span style="font-size:16px;font-weight:700;color:var(--tx1);letter-spacing:-.01em">${escapeHtml(cong.identificacao.nome)}</span>
...
function switchCongTabNamed(tabId){
  _congTabAtual=tabId;
  _activeTab=_IDX_TO_TAB.indexOf(tabId);
  document.querySelectorAll("#cong-tabs-bar .min-tab").forEach(b=>b.classList.toggle("active",b.dataset.ctab===tabId));
  const cong=CONG.getCong(_activeCongId);
  if(cong) _renderCongTabContent(tabId,cong);
}
window.switchCongTabNamed=switchCongTabNamed;

function _renderCongTabContent(tabId, cong){
  const el=document.getElementById("cong-tab-content");
  if(!el) return;
  el.innerHTML="";
  const map={
    'visao-geral': renderTab_visaoGeral,
    'membresia':   renderTab_membresia,
    'lideranca':   renderTab_lideranca,
    'cultos':      renderTab_cultos,
    'pgs':         renderTab_pgs,
    'ministerios': renderTab_ministerios,
    'financeiro':  renderTab_financeiro,
...
function renderTab_visaoGeral(cong, el){
  ...
  const IC={
    ...
    warn:  _SV('<path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3z"/><line x1="12" x2="12" y1="9" y2="13"/><line x1="12" x2="12.01" y1="17" y2="17"/>'),
  ...
  if(le.supervisao)  ldItems.push(["Supervisor",le.supervisao]);
  if(le.conselheiro) ldItems.push(["Conselheiro",le.conselheiro]);
  if(le.coordenacao) ldItems.push(["Coordenação",le.coordenacao]);
  if(le.tesoureiro)  ldItems.push(["Tesoureiro",le.tesoureiro]);
  const ldEl=document.getElementById("cong-vg-lideranca");
  if(ldEl) ldEl.innerHTML=ldItems.length
    ?ldItems.map(([l,n])=>_row(l,escapeHtml(n))).join("")
  ...
  if(cultosEl){
    const hist=(a.historico_cultos||[]).slice(0,4);
...
// ── Tab Administração ─────────────────────────────────
function renderTab_adm_cong(cong, el){
  const rec=cong.recursos||{};
  const podeEd=_podeEditar(cong.id);
  const _inpStyle="padding:8px 10px;border-radius:6px;border:1px solid var(--bd2);background:var(--bg2);color:var(--tx1);font-size:12.5px;width:100%";
  const _lbStyle="font-size:11.5px;font-weight:600;color:var(--tx3);margin-bottom:5px;display:block";

  const MODULOS=[
    {key:'cultos',       label:'Cultos e Oração',   desc:'Registro de cultos e frequência'},
    {key:'pgs',          label:'Pequenos Grupos',    desc:'Grupos e reuniões domiciliares'},
    {key:'ministerios',  label:'Ministérios',        desc:'Ministérios internos da congregação'},
    {key:'financeiro',   label:'Financeiro',         desc:'Receitas, despesas e saldo'},
  ...
  const _tog=(key,on)=>`
    <button id="cong-adm-tog-${key}" onclick="toggleCongRecurso('${cong.id}','${key}',${!on})"
      style="padding:5px 14px;border-radius:20px;border:1px solid ${on?"transparent":"var(--bd2)"};font-size:11.5px;font-weight:600;cursor:pointer;
        background:${on?"rgba(58,170,92,.14)":"transparent"};color:${on?"var(--gr)":"var(--tx3)"};transition:all .15s">
      ${on?"Ativado":"Desativado"}
    </button>`;

  el.innerHTML=`
    <div class="card" style="margin-bottom:16px">
      <div class="ctit">Informações da Congregação</div>
      <div style="display:flex;flex-direction:column;gap:14px">
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
          <div><label style="${_lbStyle}">Nome</label><input id="cong-adm-nome" value="${escapeHtml(cong.identificacao.nome)}" style="${_inpStyle}" ${podeEd?"":" disabled"}></div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px">
          <div><label style="${_lbStyle}">Pastor Responsável</label><input id="cong-adm-pastor" value="${escapeHtml(cong.lideranca.pastor_responsavel||"")}" style="${_inpStyle}" ${podeEd?"":" disabled"}></div>
        ...
        <div style="display:flex;align-items:center;gap:8px">
          <input type="checkbox" id="cong-adm-ativa" ${cong.identificacao.status==="ativa"?"checked":""} style="width:16px;height:16px;cursor:pointer" ${podeEd?"":" disabled"}>
          <label for="cong-adm-ativa" style="font-size:13px;color:var(--tx2);cursor:pointer">Congregação ativa</label>
        </div>
        ${podeEd?`<div style="display:flex;justify-content:flex-end"><button onclick="_salvarAdmCong('${cong.id}')" style="padding:9px 24px;border-radius:8px;border:none;background:var(--gr);color:#fff;font-size:13px;font-weight:600;cursor:pointer">Salvar</button></div>`:""}
      </div>
    </div>
    <div class="card">
      <div class="ctit">Módulos Opcionais</div>
      <div style="font-size:12px;color:var(--tx3);margin-bottom:14px">Ative ou desative as abas visíveis nesta congregação.</div>
      ${MODULOS.map(m=>{
        const on=rec[m.key]!==false;
        return `<div style="display:flex;align-items:center;justify-content:space-between;padding:11px 0;border-bottom:1px solid var(--bd1)">
          <div>
            <div style="font-size:13px;font-weight:600;color:var(--tx1)">${m.label}</div>
            <div style="font-size:11.5px;color:var(--tx3);margin-top:1px">${m.desc}</div>
          </div>
          ${podeEd?_tog(m.key,on):`<span style="font-size:11px;color:${on?"var(--gr)":"var(--tx4)"}">${on?"Ativado":"Desativado"}</span>`}
        </div>`;
      }).join("")}
    </div>
  `;
...
function _salvarAdmCong(id){
  const cong=CONG.getCong(id);
  if(!cong) return;
  cong.identificacao.nome=(document.getElementById("cong-adm-nome")?.value||"").trim()||cong.identificacao.nome;
  cong.identificacao.localizacao=document.getElementById("cong-adm-local")?.value||"";
  cong.identificacao.status=document.getElementById("cong-adm-ativa")?.checked?"ativa":"inativa";
  cong.lideranca.pastor_responsavel=document.getElementById("cong-adm-pastor")?.value||"";
  if(!cong.lideranca_estruturada) cong.lideranca_estruturada={};
  cong.lideranca_estruturada.supervisao=document.getElementById("cong-adm-super")?.value||"";
  cong.lideranca_estruturada.tesoureiro=document.getElementById("cong-adm-tes")?.value||"";
  CONG.saveCong(cong);
  _sbSaveCong(cong);
  buildCongMenu();
  renderCongView(cong);
  switchCongTabNamed("adm");
  if(typeof T==="function") T("Salvo","");
}
window._salvarAdmCong=_salvarAdmCong;

function toggleCongRecurso(congId, key, value){
  const cong=CONG.getCong(congId);
  if(!cong) return;
  if(!cong.recursos) cong.recursos={};
  cong.recursos[key]=value;
  CONG.saveCong(cong);
  renderCongView(cong);
  switchCongTabNamed("adm");
}
window.toggleCongRecurso=toggleCongRecurso;
